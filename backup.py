...
def backup_to_zip(folder, name, regex, verbose):
    ...
    if regex_bool is False:
        ...
        for foldername, subfolders, filenames in os.walk(folder):
            if verbose is 1 or verbose is 2:
...
# Argument handling ========================================================================================================================
parser = argparse.ArgumentParser(description="Backup files into a zip file by entering a folder location and optional regex that can be used to only include certain files.")
# Folder
parser.add_argument("-f", "--folder", dest="folder", help="Folder location to backup into a zip file. Provide relative or absolute path.", required=True, type=str)
# Zip file name
parser.add_argument("-n", "--name", dest="name", help="Name of the zip file to create. This can also be a path.", default="", type=str)
# Regex
parser.add_argument("-r", "--regex", dest="regex", help="Regex used to find files to add to zip. Must use the \"'\" character to enclose the regular expression. "
                                                        "[Example Command] `backup -f folder_name -r '\d'`", type=str, default="")
# Verbose
parser.add_argument("-v", "--verbose", help="Display extra details while running. Levels of verbosity: 0, 1 or 2. Default: 0", type=int, default=0)
